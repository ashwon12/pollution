...
public class MonthGraph extends JFrame {
	...
	public MonthGraph(String[] x, String[] data, String myGas) {
		...
		public void paint(Graphics g) {
			...
			if(x.length<10) {
				if(!myGas.equals("carbon")) {
					g.drawLine(100, 400, 90*(x.length+1), 400); // x축
				}
			}else {
				if(!myGas.equals("carbon")) {
					g.drawLine(100, 400, 940, 400); // x축
				}
			}
			...
			if(!myGas.equals("ultrafine_dust")) {
				...
				g.drawLine(100, 20, 100, 400); // y축
			}

			/*
	          * myGas =>데이터베이스 열 이름과 같게 설정
	          * 이산화질소   : nitrogen
	          * 일산화탄소   : carbon
	          * 오존      : ozone
	          * 아황산가스   : sulfur
			...
			case "nitrogen":
				...
				for(int cnt=64; cnt<400; cnt=cnt+16) {
					...
					if(x.length<10) {
						g.drawLine(100, cnt, 90*(x.length+1), cnt); // x축
					}else {
						g.drawLine(100, cnt, 940, cnt); // x축
					}
					g.setColor(new Color(0,0,0));

					Double temp = Math.round((0.042-(a*0.001))*1000) / 1000.0; // y축 값
			...
			case "ozone":
				...
				for(int cnt=64; cnt<400; cnt=cnt+16) {
					...
					Double temp = Math.round((0.031-(b*0.001))*1000) / 1000.0; // y축 값
					String str1 = Double.toString(temp);
			...
			case "carbon":
				...
				if(x.length<10) {
					g.drawLine(100, 642, 90*(x.length+1), 642); // x축
				}else {
					g.drawLine(100, 642, 940, 642); // x축
				}

				g.drawLine(100, 50, 100, 642); // y축
			...
			case "sulfur":
				...
				for(int cnt=64; cnt<400; cnt=cnt+16) {
					...
					Double temp = Math.round((0.021-(d*0.001))*1000) / 1000.0; // y축 값
					String str1 = Double.toString(temp);
			...
			case "fine_dust":
				...
				for(int cnt=65; cnt<400; cnt=cnt+16) {
					...
					Double temp = Math.round((56-(f*2))*1) / 1.0; // y축 값
					String str1 = Double.toString(temp);
			...
			case "ultrafine_dust":
				...
				for(int cnt=32; cnt<400; cnt=cnt+16) {
					...
					Double temp = Math.round((33-(e*1))*1) / 1.0; // y축 값
					String str1 = Double.toString(temp);
