...
public class GasGraph extends JFrame {
	...
	public GasGraph(String[] date, String[] area, String[] data, String[] col, String myGas, String design) {
			...
			if (design.equals("Bar")) {
				for (i = 0; i < 20; i++) {
					...
					for (int j = 0; j < 15; j++) {
						// 범위에 따라 막대 색 바꾸기
						if (col[i].equals("보통")) {
							g.setColor(new Color(240, 209, 62));
						} else if (col[i].equals("좋음")) {
							g.setColor(new Color(53, 132, 60));
				...
				for (i = 0; i < 19; i++) {
					if (col[i].equals("보통")) {
						g2.setColor(new Color(240, 209, 62));
					} else if (col[i].equals("좋음")) {
						g2.setColor(new Color(53, 132, 60));
					} else if (col[i].equals("나쁨")) {
