...
public class Gas extends JPanel {
	...
	private JButton showLineGraph, showBarGraph;
	...
	private Object[] setData(Object t_data, String selectGas) {
		...
		Object[][] r = { { "좋음", i1 }, { "보통", i2 }, { "나쁨", i3 }, { "매우나쁨", i4 } };
		switch (selectGas) {
		case "nitrogen":
			if (data <= 0.030) {
				return r[0];
			...
			if (e.getSource() == showResult) { // 결과보기 버튼을 눌렀을 경우
				...
				if (table.getValueAt(0, 1) == "") {
					...
					if (e.getSource() == showBarGraph) {
						gasGraph = new GasGraph(date, area, data, val, myGas, "Bar");
						gasGraph.setVisible(true);
					} else if (e.getSource() == showLineGraph) {
						gasGraph = new GasGraph(date, area, data, val, myGas, "Line");
						gasGraph.setVisible(true);
					}
