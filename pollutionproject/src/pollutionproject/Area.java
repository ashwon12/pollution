	...
	private class Listener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == showResult) { // 결과보기 버튼을 눌렀을 경우
				...
				temp += selectArea + "의 " + selectGas + "에 대한 <br>2018년 " + selectMonth + " " + selectDay
						+ "부터 일주일간의 농도 입니다.";
				...
				// 시작날짜와 끝 날짜를 구한다.
				String startDate = "2018-" + selectMonth.replaceAll("[^0-9]", "") + "-"
						+ selectDay.replaceAll("[^0-9]", "");
				...
				if (!isEmpty) {
					if (e.getSource() == showBarGraph) {
						areaGraph = new AreaGraph(x, data, myGas, "Bar");
						areaGraph.setVisible(true);
					} else if (e.getSource() == showLineGraph) {
						areaGraph = new AreaGraph(x, data, myGas, "Line");
						areaGraph.setVisible(true);
					}
				}
			}
