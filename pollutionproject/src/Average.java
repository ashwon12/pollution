...
public class Average extends JPanel{
	...
	private void setDate(PreparedStatement pstmt, int month) {
		try {
			switch(month+1) {
				case 1: case 3: case 5: case 7: case 8:	case 10: case 12://31일짜리
	...
	private void getSulfurMonth() {
		try {
			...
			for(int month = 0; month < 12; month++) {
				...
				//소수 넷째자리에서 반올림
				Sulfur[month] = Math.round((temp/total)*1000) / 1000.0;
			}
	...
	private void getUltrafineDustMonth() {
		try {
			ResultSet rs;
			PreparedStatement pstmt = conn.prepareStatement("SELECT date, ultrafine_dust FROM degree WHERE date between ? and ? ");
